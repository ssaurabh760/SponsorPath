/**
 * H1B Data Processing Script (Large File Version)
 * Uses ExcelJS streaming for handling large DOL files
 * 
 * Usage: node scripts/process-h1b-data-v2.js ~/Downloads/LCA_Disclosure_Data_FY2025_Q1.xlsx
 */
...
async function processH1BData(filePath) {
  ...
  console.log(`📊 Total rows: ${worksheet.rowCount.toLocaleString()}\n`);
  ...
  Object.entries(headers).forEach(([colNum, headerName]) => {
    const upper = headerName.toUpperCase();
    
    if (upper.includes('EMPLOYER_NAME') || upper === 'EMPLOYER NAME') {
      colMap.employer = parseInt(colNum);
    }
    if (upper.includes('CASE_STATUS') || upper === 'CASE STATUS') {
      colMap.status = parseInt(colNum);
    }
    if (upper.includes('JOB_TITLE') || upper === 'JOB TITLE') {
      colMap.jobTitle = parseInt(colNum);
    }
    if (upper.includes('SOC_TITLE') || upper === 'SOC TITLE') {
      colMap.socTitle = parseInt(colNum);
    }
    if ((upper.includes('WAGE_RATE_OF_PAY_FROM') || upper.includes('WAGE RATE OF PAY FROM')) && !colMap.wageFrom) {
      colMap.wageFrom = parseInt(colNum);
    }
    if ((upper.includes('WAGE_UNIT_OF_PAY') || upper.includes('WAGE UNIT OF PAY')) && !colMap.wageUnit) {
      colMap.wageUnit = parseInt(colNum);
    }
    if (upper.includes('WORKSITE_CITY') || upper === 'WORKSITE CITY') {
      colMap.city = parseInt(colNum);
    }
    if (upper.includes('WORKSITE_STATE') || upper === 'WORKSITE STATE') {
      colMap.state = parseInt(colNum);
    }
    if (upper.includes('EMPLOYER_STATE') || upper === 'EMPLOYER STATE') {
      colMap.employerState = parseInt(colNum);
    }
  });
  
  console.log('   Employer column:', colMap.employer || '❌ NOT FOUND');
  console.log('   Status column:', colMap.status || '❌ NOT FOUND');
  console.log('   Job Title column:', colMap.jobTitle || '❌ NOT FOUND');
  console.log('   Wage column:', colMap.wageFrom || '❌ NOT FOUND');
  console.log('   Wage Unit column:', colMap.wageUnit || '❌ NOT FOUND');
  console.log('   City column:', colMap.city || '❌ NOT FOUND');
  console.log('   State column:', colMap.state || colMap.employerState || '❌ NOT FOUND');
  console.log('');
  
  if (!colMap.employer) {
    // Print all headers for debugging
    console.log('\n⚠️  Could not find EMPLOYER column. Available columns:');
    Object.entries(headers).forEach(([num, name]) => {
  ...
  worksheet.eachRow((row, rowNumber) => {
    ...
    const employerRaw = row.getCell(colMap.employer).value;
    if (!employerRaw) {
      skippedRows++;
      return;
    }
    ...
    const status = String(row.getCell(colMap.status)?.value || '').toUpperCase();
    const wageFrom = row.getCell(colMap.wageFrom)?.value;
    const wageUnit = row.getCell(colMap.wageUnit)?.value;
    const wage = parseWage(wageFrom, wageUnit);
    const city = String(row.getCell(colMap.city)?.value || '').trim();
    const state = String(row.getCell(colMap.state || colMap.employerState)?.value || '').trim().toUpperCase().substring(0, 2);
    const jobTitle = String(row.getCell(colMap.jobTitle || colMap.socTitle)?.value || '').trim();
...
function generateSQL(companies) {
  const lines = [
    ...
    '-- DELETE FROM public.company_stats;',
...
if (args.length === 0) {
  console.log('Usage: node scripts/process-h1b-data-v2.js <path-to-xlsx-file>');
  console.log('Example: node scripts/process-h1b-data-v2.js ~/Downloads/LCA_Disclosure_Data_FY2025_Q1.xlsx');
  process.exit(1);
}
